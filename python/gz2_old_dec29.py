...
def create_binned_data_rawlikelihood(task_dict,
    ...
    magnitude_mask = mr > (absmag_lim - magpadding)
    size_mask = r90_kpc < (3.0 * ang_scale)
    surfacebrightness_mask = (mr + dmod + \
        2.5*np.log10(6.283185*(r50_kpc/ang_scale)**2)) > (SBlim_app - sbpadding)
    ...
    tend = time.time()
    print 'Time elapsed : %i seconds' % (tend - tstart)
...
def plot_ratio_baseline_redshift(task_dict):
    ...
    if bintype is 'rawlikelihood':
        ...
        centers_redshift, centers_mag, centers_size = get_bins(task_dict)
    ...
    if len(zbins) < 24:
        ...
        maxzind = 24
    for idx,z in enumerate(zbins[:maxzind]):
        ...
        im = ax.imshow(imarr_mask.T, 
                       extent=(np.min(magbins),np.max(magbins),np.min(sizebins),np.max(sizebins)), 
...
def create_binned_data(task_dict,
    ...
    if task_dict['wp_type'] is 'weighted':
        ...
        fig.clf()
        ax = fig.add_subplot(111)
        hist(wp, bins='scotts', ax=ax, histtype='step', color='b',normed=True)
...
def plot_ratio_baseline(task_dict,
    ...
    for index, data in enumerate((sumcounts, ratio_baseline_masked)):
        ...
        SBlim_mag = (SBlim_app - cosmology.dmod_flat(np.mean(zbins))- 2.5*np.log10(6.283185*(SBlim_size/cosmology.ang_scale_flat(np.mean(zbins)))**2))
        ...
        size_1arcsec = cosmology.ang_scale_flat(np.mean(zbins))
        ax.autoscale(False)
        ax.plot(SBlim_mag, SBlim_size,'w--')
        if plot_mag_lims:
            ax.axvline(absmag_lim,color='w',linestyle='dashed')
...
def fit_ratio_baseline(task_dict, nboot=50, plot=False, unset_vrange=False):
    ...
    if plot:
        plot_ratio_function(p,magbins,sizebins, unset_vrange)
...
def plot_ratio_baseline_fit(task_dict, 
    ...
    if match_databins:
        magbinsplot,sizebinsplot = magbins,sizebins
    else:
        magbinsplot,sizebinsplot = np.linspace(magedges[0],magedges[-1],fitbins),np.linspace(sizeedges[0],sizeedges[-1],fitbins)

    fitarray_fn = ratio_function(pfit, magbinsplot, sizebinsplot)
    #fitarray_fn = ratio_function(pfit, magbins, sizebins)
...
def run_task(task_dict,
    ...
    if plot:
        ...
        plot_baseline_correction_slice(task_dict)
...
def run_all_tasks(bintype='counts'):
...
def determine_baseline_correction(task_dict, direct=False, spb=False):
    ...
    pfit = ratio_function(pfit_params, magbins, sizebins)
...
def plot_baseline_correction(task_dict,direct=False,vrange=(min_ratio,max_ratio)):

    c,cmasked, ratio_masked = determine_baseline_correction(task_dict,direct=direct)
    ...
    if bintype is 'counts':
        ...
        zbins = p_var1['REDSHIFT_BIN_CENTERS'].data['centers']
        magedges = p_var1['MR_BIN_EDGES'].data['edges']
        sizeedges = p_var1['R50_KPC_BIN_EDGES'].data['edges']
    ...
    if bintype is 'rawlikelihood':
        ...
        zbins = p_allvar['REDSHIFT_BIN_CENTERS'].data['centers']
        magedges = p_allvar['MR_BIN_EDGES'].data['edges']
        sizeedges = p_allvar['R50_KPC_BIN_EDGES'].data['edges']
    ...
    for idx,z in enumerate(zbins[:24]):
        ...
        im = ax.imshow(cmasked[idx,:,:].T,
                       extent=(magedges[0],magedges[-1],sizeedges[0],sizeedges[-1]),
                       vmin=vrange[0],vmax=vrange[1],
                       interpolation='nearest',origin='lower')
        ax.set_aspect('auto')
    ...
    fig.text(0.5,0.95,'Baseline correction per redshift bin for %s' % task_dict['var_def'], fontsize=22, ha='center')

    fig.savefig(plots_path+'%s_baseline_correction.png' % task_dict['var_def'], dpi=200)
...
def plot_baseline_correction_slice(task_dict, bslice=5,
    ...
    zbins = p1_counts['REDSHIFT_BIN_CENTERS'].data['centers']
    magbins = p1_counts['MR_BIN_CENTERS'].data['centers']
    sizebins = p1_counts['R50_KPC_BIN_CENTERS'].data['centers']

    zedges = p1_counts['REDSHIFT_BIN_EDGES'].data['edges']
    magedges = p1_counts['MR_BIN_EDGES'].data['edges']
    sizeedges = p1_counts['R50_KPC_BIN_EDGES'].data['edges']
    ...
    im1 = ax1.imshow(zslice.T,
                     ...
                     vmin = vmin, vmax = vmax,
                     interpolation='nearest',origin='lower')
    ax1.set_title('Data ratio, %5.3f < z < %5.3f' % (zbins[bslice]-zstep/2.,zbins[bslice]+zstep/2.))
    ax1.set_aspect('auto')
    cb = fig.colorbar(im1)
    cb.set_label('ratio')
    ...
    pfit, pfit_err = pickle.load(file(pkl_path+'%s_ratio_baseline_fit.pkl' % var_def, 'r'))
    if smoothfunc:
        fit = ratio_function(pfit, np.linspace(magedges[0],magedges[-1],1000),np.linspace(sizeedges[0],sizeedges[-1],1000))
    else:
        fit = ratio_function(pfit, magbins, sizebins)
    fit_extent=(magedges[0],magedges[-1],sizeedges[0],sizeedges[-1])
...
def adjust_probabilities(task_dict, plot=False, direct=False, stripe82=False):
    ...
    correction, correction_masked, ratio_masked = determine_baseline_correction(task_dict, direct = direct)
    ...
    if bintype is 'counts':
        ...
        zedges = p_var1['REDSHIFT_BIN_EDGES'].data['edges']
        magedges = p_var1['MR_BIN_EDGES'].data['edges']
        sizeedges = p_var1['R50_KPC_BIN_EDGES'].data['edges']
    ...
    if bintype is 'rawlikelihood':
        ...
        zedges = p_allvar['REDSHIFT_BIN_EDGES'].data['edges']
        magedges = p_allvar['MR_BIN_EDGES'].data['edges']
        sizeedges = p_allvar['R50_KPC_BIN_EDGES'].data['edges']
    ...
    zstep = zedges[1] - zedges[0]
    magstep = magedges[1] - magedges[0]
    sizestep = sizeedges[1] - sizeedges[0]
    ...
    for idx, (el, sp) in enumerate(zip(p_el_raw, p_sp_raw)):
        ...
        zbin = np.abs(z2 >= zedges).argmin() - 1
        mbin = np.abs(m2 >= magedges).argmin() - 1
        sbin = np.abs(s2 >= sizeedges).argmin() - 1
    ...
    print 'Time elapsed: %i seconds' % (timeend2 - timestart2)
...
def plot_galaxy_counts(task_dict,min_classifications=20,magstep=0.25,sizestep=0.5, vmax=1000):
    ...
    zstep=0.01
    zarr = np.arange(0.00,0.25,zstep)
    magmin,magmax = -24,-17
    sizemin,sizemax = 0,15
    ...
    for idx,z in enumerate(zarr[:24]):
...
def plot_type_fractions(task_dict, zlimval=0.085, stripe82=False):
    ...
    zwidth = 0.02
    zplotbins = np.arange(0.00,0.26,zwidth)

    p_el_raw_binned = np.zeros_like(zplotbins)
    p_sp_raw_binned = np.zeros_like(zplotbins)
    p_el_adj_binned = np.zeros_like(zplotbins)
    p_sp_adj_binned = np.zeros_like(zplotbins)
    p_el_raw_binned_maglim = np.zeros_like(zplotbins)
    p_sp_raw_binned_maglim = np.zeros_like(zplotbins)
    p_el_adj_binned_maglim = np.zeros_like(zplotbins)
    p_sp_adj_binned_maglim = np.zeros_like(zplotbins)
    ...
    for idx,zbin in enumerate(zplotbins):
        ...
        p_el_raw_binned[idx] = np.mean(p_el_raw_values[gals_in_bin_raw])
        p_sp_raw_binned[idx] = np.mean(p_sp_raw_values[gals_in_bin_raw])

        gals_in_bin_adj = (redshift_adj >= zbin) & (redshift_adj < (zbin+zwidth))
        weights_adj = task_counts_adj[gals_in_bin_adj]
        p_el_adj_binned[idx] = np.mean(p_el_adj_values[gals_in_bin_adj])
        p_sp_adj_binned[idx] = np.mean(p_sp_adj_values[gals_in_bin_adj])
        ...
        p_el_raw_binned_maglim[idx] = np.sum(p_el_raw_values_maglim[gals_in_bin_maglim_raw] * weights_maglim_raw) / np.sum(weights_maglim_raw)
        p_sp_raw_binned_maglim[idx] = np.sum(p_sp_raw_values_maglim[gals_in_bin_maglim_raw] * weights_maglim_raw) / np.sum(weights_maglim_raw)
        ...
        p_el_adj_binned_maglim[idx] = np.sum(p_el_adj_values_maglim[gals_in_bin_maglim_adj] * weights_maglim_adj) / np.sum(weights_maglim_adj)
        p_sp_adj_binned_maglim[idx] = np.sum(p_sp_adj_values_maglim[gals_in_bin_maglim_adj] * weights_maglim_adj) / np.sum(weights_maglim_adj)

    pickle.dump(p_el_raw_binned, open(pkl_path+'%s_%s_%sraw_binned.pkl' % (var_def,var1_str,s82_str),'wb')) 
    pickle.dump(p_sp_raw_binned, open(pkl_path+'%s_%s_%sraw_binned.pkl' % (var_def,var2_str,s82_str),'wb')) 
    pickle.dump(p_el_adj_binned, open(pkl_path+'%s_%s_%sadj_binned.pkl' % (var_def,var1_str,s82_str),'wb')) 
    pickle.dump(p_sp_adj_binned, open(pkl_path+'%s_%s_%sadj_binned.pkl' % (var_def,var2_str,s82_str),'wb')) 
    pickle.dump(p_el_raw_binned_maglim, open(pkl_path+'%s_%s_%sraw_binned_maglim.pkl' % (var_def,var1_str,s82_str),'wb')) 
    pickle.dump(p_sp_raw_binned_maglim, open(pkl_path+'%s_%s_%sraw_binned_maglim.pkl' % (var_def,var2_str,s82_str),'wb')) 
    pickle.dump(p_el_adj_binned_maglim, open(pkl_path+'%s_%s_%sadj_binned_maglim.pkl' % (var_def,var1_str,s82_str),'wb')) 
    pickle.dump(p_sp_adj_binned_maglim, open(pkl_path+'%s_%s_%sadj_binned_maglim.pkl' % (var_def,var2_str,s82_str),'wb')) 
    ...
    ax1.plot(zplotbins, p_el_raw_binned, color='r', linestyle='-' ,linewidth=2)
    ax1.plot(zplotbins, p_sp_raw_binned, color='b', linestyle='--',linewidth=2)
    ax1.plot(zplotbins, p_el_adj_binned, color='r', linestyle='-' ,linewidth=4)
    ax1.plot(zplotbins, p_sp_adj_binned, color='b', linestyle='--',linewidth=4)
    ...
    ax2.plot(zplotbins, p_el_raw_binned_maglim, color='r', linestyle='-' ,linewidth=2)
    ax2.plot(zplotbins, p_sp_raw_binned_maglim, color='b', linestyle='--',linewidth=2)
    ax2.plot(zplotbins, p_el_adj_binned_maglim, color='r', linestyle='-' ,linewidth=4)
    ax2.plot(zplotbins, p_sp_adj_binned_maglim, color='b', linestyle='--',linewidth=4)
...
def plot_all_type_fractions(zlimval=0.085, stripe82=False):
    ...
    for idx,task_dict in enumerate(tasklist):
        ...
        p_el_raw_binned        = pickle.load(file(pkl_path+'%s_%s_%sraw_binned.pkl' % (var_def,var1_str,s82_str),'rb')) 
        p_sp_raw_binned        = pickle.load(file(pkl_path+'%s_%s_%sraw_binned.pkl' % (var_def,var2_str,s82_str),'rb')) 
        p_el_adj_binned        = pickle.load(file(pkl_path+'%s_%s_%sadj_binned.pkl' % (var_def,var1_str,s82_str),'rb')) 
        p_sp_adj_binned        = pickle.load(file(pkl_path+'%s_%s_%sadj_binned.pkl' % (var_def,var2_str,s82_str),'rb')) 
        p_el_raw_binned_maglim = pickle.load(file(pkl_path+'%s_%s_%sraw_binned_maglim.pkl' % (var_def,var1_str,s82_str),'rb')) 
        p_sp_raw_binned_maglim = pickle.load(file(pkl_path+'%s_%s_%sraw_binned_maglim.pkl' % (var_def,var2_str,s82_str),'rb')) 
        p_el_adj_binned_maglim = pickle.load(file(pkl_path+'%s_%s_%sadj_binned_maglim.pkl' % (var_def,var1_str,s82_str),'rb')) 
        p_sp_adj_binned_maglim = pickle.load(file(pkl_path+'%s_%s_%sadj_binned_maglim.pkl' % (var_def,var2_str,s82_str),'rb')) 
    ...
    ax2.plot(zplotbins, p_el_raw_binned_maglim-100, color='r', linestyle='-' ,linewidth=2)
    ax2.plot(zplotbins, p_sp_raw_binned_maglim-100, color='b', linestyle='--',linewidth=2)
    ax2.plot(zplotbins, p_el_adj_binned_maglim-100, color='r', linestyle='-' ,linewidth=4)
    ax2.plot(zplotbins, p_sp_adj_binned_maglim-100, color='b', linestyle='--',linewidth=4)

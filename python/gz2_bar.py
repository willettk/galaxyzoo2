...
def create_binned_data(min_class=20,min_prob=0.8,
    ...
    # Change sp to bar, el to nobar

    taskcount = gzdata['t03_bar_total_weight'] > min_class
    bar_prob  = gzdata['t03_bar_a06_bar_weighted_fraction'] > min_prob
    nobar_prob  = gzdata['t03_bar_a07_no_bar_weighted_fraction'] > min_prob
    bar = gzdata[bar_prob & taskcount]
    nobar = gzdata[nobar_prob & taskcount]

    print ''
    print '%s barred galaxies above threshold'%len(bar)
    print '%s non-barred galaxies above threshold'%len(nobar)
    print ''
...
def determine_ratio_baseline(min_count=50,linear=False):

    # Load the GZ2 sample data

    p_nobar = pyfits.open(fits_path+'nobar_binned_counts.fits')
    p_bar = pyfits.open(fits_path+'bar_binned_counts.fits')

    d_nobar = p_nobar[0].data.astype(int)                         # Data is pre-binned
    d_bar = p_bar[0].data.astype(int)

    # Bin sizes are set when FITS file is created

    zbins = p_nobar['REDSHIFT_BIN_CENTERS'].data['centers']
    magbins = p_nobar['MR_BIN_CENTERS'].data['centers']
    sizebins = p_nobar['R50_KPC_BIN_CENTERS'].data['centers']
    ...
    for z_index, zsel in enumerate(zbins[1:]):
        nobar = nobarz[z_index, :, :]
        bar = barz[z_index, :, :]
     
        # Create mask for cells with low counts
        mask = (bar + nobar) < min_count
     
        # Compute the elliptical/spiral ratio for the entire array

        ratio_temp = nobar.astype(np.float)/bar
        if linear:
            ratio = ratio_temp
        else:
            ratio = np.log10(ratio_temp)
        ...
        counts_baseline[select] = np.transpose([nobar[select], bar[select]])
    ...
    pyfits.writeto(fits_path+'local_ratio_baseline_bar.fits',
                   ratio_baseline, clobber=True)    
    pyfits.writeto(fits_path+'local_counts_baseline_bar.fits',
                   counts_baseline, clobber=True)    

    pickle.dump(ratio_baseline_masked, open(gz_path+'local_ratio_baseline_bar_masked.pkl','wb')) 
    pickle.dump(counts_baseline_masked, open(gz_path+'local_counts_baseline_bar_masked.pkl','wb')) 
...
def determine_ratio_baseline_sigma(plot=False):
    data = pyfits.getdata(fits_path+'local_counts_baseline_bar.fits')
    ...
    ratio = pyfits.getdata(fits_path+'local_ratio_baseline_bar.fits')
    ...
    pyfits.writeto(fits_path+'local_ratio_baseline_bar_sigma.fits',
           sigma, clobber=True)    
...
def plot_ratio_baseline(plot_mag_lims=False):
    ...
    f_ratio = fits_path+'local_ratio_baseline_bar.fits'
    f_counts = fits_path+'nobar_binned_counts.fits'
    ...
    ratio_baseline_masked = pickle.load(open(gz_path+'local_ratio_baseline_bar_masked.pkl','rb'))
    counts_baseline_masked = pickle.load(open(gz_path+'local_counts_baseline_bar_masked.pkl','rb'))

    sumcounts = np.sum(counts_baseline_masked, axis=2)

    titlenames = ('counts','ratio')
    labelnames = (r'$N_{nobar} + N_{bar}$',r'$log_{10}(N_{nobar}/N_{bar})$')

    fig = plt.figure(3,(14,8))
    fig.clf()
    vrange = [(np.min(sumcounts),np.max(sumcounts)),(-2.,2.)]
    vrange = [(np.min(sumcounts),np.max(sumcounts)),(-0.4,1.2)]
    vrange = [(np.min(sumcounts),np.max(sumcounts)),(np.min(ratio_baseline_masked),np.max(ratio_baseline_masked))]
    ...
    for index, data in enumerate((sumcounts, ratio_baseline_masked)):
        ...
        ax.set_title('bar/nobar '+titlenames[index])
        ax.set_xlabel(r'$M_R [mag]$',fontsize=22)
        ax.set_ylabel(r'$R_{50} [kpc]$',fontsize=22)
        ax.set_aspect('auto')
...
def fit_ratio_baseline(nboot=50, plot=False):
    ...
    sigma = pyfits.getdata(fits_path+'local_ratio_baseline_bar_sigma.fits')
    ratio = pyfits.getdata(fits_path+'local_ratio_baseline_bar.fits')
    ...
    pickle.dump((p, perr), file(fits_path+'ratio_baseline_bar_fit.pickle', 'w'))
    res = ratio - ratio_baseline_fit
    normres = res/sigma
    np.putmask(normres, weights < 0.000001, unknown_ratio)

    if plot:
        plot_ratio_function(p,magbins,sizebins)

    pyfits.writeto(fits_path+'local_ratio_baseline_bar_fit.fits',
                   ratio_baseline_fit, clobber=True)    
    pyfits.writeto(fits_path+'local_ratio_baseline_bar_fitres.fits',
                   res, clobber=True) 
    pyfits.writeto(fits_path+'local_ratio_baseline_bar_fitnormres.fits',
...
def plot_ratio_baseline_fit(fitbins=19, kernel_size = 3, plot_contour=False):
    ...
    ratio_baseline_fit = pyfits.getdata(fits_path+'local_ratio_baseline_bar_fit.fits')
    ...
    pfit, pfit_err = pickle.load(file(fits_path+'ratio_baseline_bar_fit.pickle', 'r'))
...
def do_all(min_count=20, nboot=50, min_class=20, min_prob=0.8, fitbins = 1000):
    
    tstart = time.time()

    create_binned_data(min_class,min_prob)
    determine_ratio_baseline(min_count,linear=True)
    plot_ratio_baseline()

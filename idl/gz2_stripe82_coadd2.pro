...
pro gz2_stripe82_coadd2, ps=ps

; Import Stripe 82 data

gz2dir = '~/Astronomy/Research/GalaxyZoo/'
gz2tablefile = gz2dir+'fits/gz2table.fits'
gz2samplefile = gz2dir+'fits/gz2sample.fits'
...
gz2_n_c2_coadd_file = gz2dir+'fits/TEST_n_c2_coadd_willettk.fit'
gz2_n_c2_normal_file = gz2dir+'fits/TEST_n_c2_normal_willettk.fit'
gz2_n_c2_spec_file = gz2dir+'fits/TEST_n_c2_spec_willettk.fit'
gz2_n_c2_sample_file = gz2dir+'fits/TEST_n_c2_sample_willettk.fit'
...
temparr1=intarr(nt) & for i=0,nt-1 do begin & temp = tagnames[i] & templen = strlen(temp) & if templen ge 20 then begin & temp2 = strmid(temp,templen-19,19) & if temp2 eq 'WEIGHTED_FRACTION_1' then temparr1[i]=1 & endif & endfor
temparr2=intarr(nt) & for i=0,nt-1 do begin & temp = tagnames[i] & templen = strlen(temp) & if templen ge 20 then begin & temp2 = strmid(temp,templen-19,19) & if temp2 eq 'WEIGHTED_FRACTION_2' then temparr2[i]=1 & endif & endfor
...
for i=0,nbyn^2-1 do begin 
	...
	countind = where(nc2_coadd.(tagind_c) ge count and nc2_normal.(tagind_c) ge count)
	coadd2_val = nc2_coadd[countind].(tagind_f)
	normal_val = nc2_normal[countind].(tagind_f)
	z = nc2_spec[countind].z
...
for i=0,nbyn^2-1 do begin 
	...
	countind = where(nc2_coadd.(tagind_c) ge count and nc2_normal.(tagind_c) gt count)
	coadd2_val = nc2_coadd[countind].(tagind_f)
	normal_val = nc2_normal[countind].(tagind_f)
	z = nc2_spec[countind].z

...
if keyword_set(ps) then ps_end
    ...
    for i=0,n_elements(wfarr)-1 do $
        cghistoplot,gz2_main[where(gz2_main.t01_smooth_or_features_a02_features_or_disk_weighted_fraction ge wfarr[i] $ 
        and gz2_main.t01_smooth_or_features_total_weight ge t01lim)].t02_edgeon_total_weight,$
    ...
    ; Keep same limit for Tasks 05, 10 and 11 - tree doesn't branch
    t04lim=10
    for i=0,n_elements(wfarr)-1 do $
    ...
    for i=0,n_elements(wfarr)-1 do $
        cghistoplot,gz2_main[where(gz2_main.t04_spiral_a08_spiral_weighted_fraction ge wfarr[i] $ 
        and gz2_main.t04_spiral_total_weight ge t04lim)].t05_bulge_prominence_total_weight,$
        binsize=1,/outline,yr=[0,10000],title='Task 04 threshold on Task 05: '+strtrim(wfarr[i],2),charsize=2,xr=[0,60]
    ...
    for i=0,n_elements(wfarr)-1 do $
        ...
        binsize=1,/outline,yr=[0,10000],title='Task 02 threshold on Task 09: '+strtrim(wfarr[i],2),charsize=2,xr=[0,60]
    ...
    for i=0,n_elements(wfarr)-1 do $
        ...
        binsize=1,/outline,yr=[0,35000],title='Task 01 threshold on Task 08: '+strtrim(wfarr[i],2),charsize=2,xr=[0,60]

...
%DIF DEL gz2_circulated_version.tex   Wed Jun  5 09:05:11 2013
%DIF ADD gz2datarelease_pdf_cjl.tex   Mon Jun 17 14:51:39 2013
%
...
Whereas traditional morphological classification relied on the \DIFdelbegin \DIFdel{careful }\DIFdelend inspection of small numbers of images by experts \citep[e.g., ][]{san61,dev91}, the sheer size of modern data sets make this approach impractical. Detailed classifications of SDSS images by experts have been \DIFdelbegin \DIFdel{done }\DIFdelend \DIFaddbegin \DIFadd{carried out }\DIFaddend by both \citet{fuk07} and \citet{bai11}, who determined modified Hubble types for samples of 2253 and 4458 galaxies, respectively. The largest detailed professional effort to date was undertaken by \citet{nai10}, who provide classifications of 14,034 galaxies. Galaxy~Zoo~2 \DIFdelbegin \DIFdel{includes more than }\DIFdelend \DIFaddbegin \DIFadd{produces a catalogue which is }\DIFaddend an order of magnitude \DIFdelbegin \DIFdel{more systems, each with }\DIFdelend \DIFaddbegin \DIFadd{bigger, with each galaxy receiving }\DIFaddend a large number of independent inspections. The size of this sample allows for a more complete study of small-scale morphological features and better statistics for rarer classes of objects, while having many independent classifications for each galaxy yields estimates of the classification likelihood (and possibly the relative feature strength). 

The use of proxies for morphology --- such as colour, concentration index, spectral features, surface brightness profile, structural features, spectral energy distribution or some combination of these --- is not an adequate substitute. Each proxy has an unknown and \DIFdelbegin \DIFdel{possibly }\DIFdelend \DIFaddbegin \DIFadd{likely }\DIFaddend biased relation with the morphological features under study. With a sufficiently large set of galaxies, however, the diversity of the local population can be fully sampled, quantifying the relationship between morphology and the proxies discussed above. 
...
An error in the selection query meant that the \DIFdelbegin \DIFdel{``original'' }\DIFdelend \DIFaddbegin \DIFadd{`original' }\DIFaddend sample initially missed objects flagged by the SDSS pipeline as having both \textsc{blended} and \textsc{child} morphologies. These galaxies, which are typically slightly brighter, larger and bluer than the general population, were added to the GZ2 site on 2009-09-02. These additional 28,174 galaxies are referred to as the \DIFdelbegin \DIFdel{``extra'' }\DIFdelend \DIFaddbegin \DIFadd{`extra' }\DIFaddend sample. 

In addition to galaxies from the DR7 Legacy, GZ2 also classified images from Stripe~82, a multiply-imaged section along the celestial equator in the Southern Galactic Cap. The selection criteria were the same as for the Legacy galaxies, with the exception of a fainter magnitude limit of $m_r < 17.77$. For the Stripe~82 sample only, GZ2 includes multiple images of individual galaxies: one set of images at \DIFdelbegin \DIFdel{single-depth exposures}\DIFdelend \DIFaddbegin \DIFadd{single-exposure depth}\DIFaddend , plus two sets of co-added images from multiple exposures. Coadded images combined 47 (south) or 55 (north) separate scans of the region, resulting in an object detection limit approximately two magnitudes lower than in normal imaging \citep{ann11}. 

The primary sample for GZ2 analysis consists of the combined \DIFdelbegin \DIFdel{``original'', ``extra''}\DIFdelend \DIFaddbegin \DIFadd{`original', `extra'}\DIFaddend , and Stripe~82 normal-depth images with $m_r\leq17.0$. We have verified that there are no significant differences in the classifications between these sub-samples (\DIFdelbegin \DIFdel{possibly due to a time-dependent bias , for example}\DIFdelend \DIFaddbegin \DIFadd{i.e. no significant bias is introduced by the fact they were classified at different times}\DIFaddend ) and thus can be reliably used as a single data set. This is hereafter referred to as the GZ2 {\bf main sample} (Table~\ref{tbl-sample}), and is used for the bulk of the analysis in this paper. Data from both the Stripe~82 normal-depth images with $m_r>17.0$ and the two sets of coadded images are separately included in this data release. 
...
\begin{itemize}
	\item Galaxy~Zoo~1 \citep{lin11} \DIFaddbegin \DIFadd{: Citizen Science
	}\DIFaddend \item \citet{nai10} \DIFaddbegin \DIFadd{: Expert visual classification
	}\DIFaddend \item EFIGI \citep{bai11} \DIFaddbegin \DIFadd{: Expert visual classification
	}\DIFaddend \item \citet{hue11} \DIFaddbegin \DIFadd{: Automatic classification
...
\hline
 \end{tabular}
 \caption{Comparison of the agreement in morphology between the GZ2 main sample and other \DIFdelbeginFL \DIFdelFL{catalogs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{catalogues}\DIFaddendFL . For each category, a galaxy is considered to ``agree'' if it has a likelihood of at least 0.8 (``clean'') in GZ2 and at least 0.5 (``majority'') in the other \DIFdelbeginFL \DIFdelFL{catalog }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{catalogue }\DIFaddendFL (or for NA10, inclusion in the relevant category). This table gives both the total number of GZ2 galaxies and the fraction that agree in the corresponding \DIFdelbeginFL \DIFdelFL{catalog}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{catalogue}\DIFaddendFL . \label{tbl-compare}}
\end{table}
...
\citet[][HC11]{hue11} have generated a large set of morphological classifications for the SDSS spectroscopic sample using \DIFdelbegin \DIFdel{a Bayesian automated routine}\DIFdelend \DIFaddbegin \DIFadd{an automated Bayesian approach}\DIFaddend . The broad nature of their probabilities (four broad morphological categories), do not directly relate to the majority of the GZ2 fine structure questions, such as bar or spiral arm structure. Comparison between the two samples, however, is useful to demonstrate the effect that smaller-scale features \DIFaddbegin \DIFadd{as classified by Galaxy Zoo }\DIFaddend may have on automatically-classified morphologies.

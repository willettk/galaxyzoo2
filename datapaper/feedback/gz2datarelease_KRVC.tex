...
\newcommand{\krvc}[1]{\textcolor{red}{\textbf{[S, 6Jan: #1]}}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%   Angular Separation Bias   %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Angular Separation Bias}\label{ang-bias}

\krvc{The vote fractions also suffer from a bias which depends on the angular separation between galaxies.  For some classifications participants perceive a galaxy`s morphology differently when it has a close apparent companion.  \citet{cas13} found that this bias is particularly strong for task \it ``Anything odd?'', and in particular the ``merger'' classification.  The mean merger vote fractions of both physically close galaxies with similar redshifts and projected pairs with very different redshifts where found to increase strongly as a function of decreasing angular separation.  This results in projected pairs of non-interacting galaxies being classified as mergers.  To determine an unbiased estimate of the mean probability for a given classification, \cite{cas13} subtract the mean probabilities of projected pairs (with very different redshifts) from physically close pairs (with similar redshifts) for each projected separation bin.  This results in a residual probability which is considered to represent the true change in morphology due to strong tidal interaction.  While such a correction can be applied in a statistical way to the mean vote fractions for a given classification,  applying such a correction to the individual vote fractions is not as straight forward.

The vote fractions presented in this data release have not been corrected for angular separation bias and readers using GZ data to study very close pairs are advised to keep in this in mind, particularly for the  task \it ``Anything odd?''.  Fortunately, the angular separation bias is minimal for the rest of the classifications and can usually be ignored.  For a  detailed discussion of the angular separation bias, and how individual classifications are affected, the reader is referred to \citet{cas13}.}
...
Tables~\ref{tbl-mainclass}--\ref{tbl-stripe82_coadd2} can also be accessed from CasJobs in the SDSS Data Release~10, expected to be available as of Jul~2013. 
...
\krvc{In addition to the angular separation bias discussed in section \ref{ang-bias}, the classifications in the task {\it ``Anything odd?''} also suffer from crosstalk between the different possible options.  This is a result of more than one of the {\it ``Anything odd?''} classifications being applicable to a given galaxy, which forces the participant to choose the one they consider most relevant.  For example, a merging galaxy may display a strong dust lane, be highly irregular in shape, and have a disturbed appearance.  While ``merger'', ``dust lane'', ``irregular'' and ``disturbed'' are all possible classifications, the participant will usually choose the ``merger'' classification and information about the other morphological features will be lost.  For close pairs this crosstalk is a function of angular separation, and as ``merger'' class increases with decreasing separation, the other  {\it ``Anything odd?''} classifications loose votes \citep{cas13}.

For the other GZ2 tasks the classifications are mutually exclusive and only one of the choice is applicable.  Crosstalk is  present in the GZ1 classifications, specifically the merger class.  In the latest incarnation of Galaxy Zoo (GZ4) it is possible to select multiple classifications from the Odd task so crosstalk should be avoided.}

...
%DIF DEL kww.tex   Sun Jun 16 13:18:18 2013
%DIF ADD spb.tex   Sun Jun 16 13:18:36 2013
%
...
%DIF -------
\newcommand{\dcoadd}{$\Delta_\mathrm{coadd}$} %DIF > 
%DIF -------
\newcommand{\mr}{$M_r$}
...
The Galaxy~Zoo project \citep{lin08} \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{originally }\DIFaddend launched in 2007 \DIFdelbegin \DIFdel{to provide }\DIFdelend \DIFaddbegin \DIFadd{with the goal of obtaining }\DIFaddend morphological classifications for \DIFaddbegin \DIFadd{the }\DIFaddend nearly one million galaxies drawn from the Sloan Digital Sky Survey \citep[SDSS;][]{yor00} \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{Main Galaxy Sample (Strauss et al. 2002). }\DIFaddend This scale of effort was made possible by combining classifications from hundreds of thousands of volunteers \DIFdelbegin \DIFdel{, but in order to keep the task at a manageable level of complexity only simple }\DIFdelend \DIFaddbegin \DIFadd{via an internet site}\footnote{\DIFadd{Archived at }\url{http://zoo1.galaxyzoo.org}}\DIFadd{. As the first ever experiment in volunteer galaxy morphology classification, only the most basic }\DIFaddend morphological distinctions were \DIFdelbegin \DIFdel{initially requested, primarily dividing systems into elliptical, spiraland merger. Following }\DIFdelend \DIFaddbegin \DIFadd{requested, enabling the separation of systems into basic categories of ``elliptical" (or ``early-type") and ``spiral" (or ``late-type") galaxies, and the identification of merging systems. However, following }\DIFaddend the success of \DIFdelbegin \DIFdel{the original project , we applied the same method to }\DIFdelend \DIFaddbegin \DIFadd{this project (for which the data was publicly released in Lintott et al. 2011), the same methodology of asking for volunteer classifications was launched in 2009 with }\DIFaddend a more complex classification system. This paper presents data and results from \DIFaddbegin \DIFadd{this second incarnation of Galaxy Zoo, which we will call }\DIFaddend Galaxy\DIFdelbegin \DIFdel{Zoo's successor, called Galaxy}\DIFdelend ~Zoo~2 (GZ2)\DIFdelbegin \DIFdel{, comprising }\DIFdelend \DIFaddbegin \footnote{\DIFadd{The Galaxy Zoo 2 site is archived at }\url{http://zoo2.galaxyzoo.org}}\DIFadd{. These data comprise }\DIFaddend detailed morphologies for more than 300,000 of the largest and brightest \DIFdelbegin \DIFdel{SDSS galaxies.}\footnote{%DIFDELCMD < \url{http://zoo2.galaxyzoo.org}%%%
}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{of the SDSS MGS galaxies.
}\DIFaddend 

While the morphological distinction used in the original Galaxy~Zoo (GZ1) -- that which divides spiral and elliptical systems -- is the most fundamental, \DIFdelbegin \DIFdel{there }\DIFdelend \DIFaddbegin \DIFadd{the motivation for Galaxy Zoo 2 was that galaxies demonstrate a much wider variety of morphological features. There }\DIFaddend is a long history of enhanced classifications \DIFdelbegin \DIFdel{. \mbox{%DIFAUXCMD
\citet{hub36}
}%DIFAUXCMD
created one of the first systematic approaches to classification by including }\DIFdelend \DIFaddbegin \DIFadd{(see Buta 2011 for a recent historical review), but the most well known approach to classification \mbox{%DIFAUXCMD
\citep{hub26,hub36}
}%DIFAUXCMD
included }\DIFaddend a division between barred and unbarred spirals, \DIFdelbegin \DIFdel{resulting in }\DIFdelend \DIFaddbegin \DIFadd{creating }\DIFaddend the famous `tuning fork' diagram. Further distinctions \DIFdelbegin \DIFdel{were based on the shape of early-type systems or tightness of late-type spiral arms . These traits are often }\DIFdelend \DIFaddbegin \DIFadd{ordered ellipticals based on their shape and spirals based on a combination of the tightness and distinction of spiral arms and size of central bulge. Along the spiral or late-type sequence, these finer morphological distinctions are found to be }\DIFaddend correlated with physical parameters of the systems being studied \DIFdelbegin \DIFdel{; galactic bars, for example, can drive gas inwards and are related to the growth of a central bulge \mbox{%DIFAUXCMD
\citep[reviews are given in \citealt{kor04} and][]{mas11c}
}%DIFAUXCMD
. Similarly, }\DIFdelend \DIFaddbegin \DIFadd{(Robert }\& \DIFadd{Haynes 1994), with spirals becoming, on average, redder, more massive and less gas-rich the ``earlier" (closer to the ``early-types" in the sequence) they are. 
}

\DIFadd{It is clear that morphological features provide insights into the physical processes that have shaped the evolution of galaxies. Most obviously, merger features reveal ongoing gravitational interactions, but even }\DIFaddend the presence of a central bulge \DIFaddbegin \DIFadd{in a disc galaxy }\DIFaddend is likely to indicate a history of mass assembly through significant mergers (\citealt{mar12} and references therein). \DIFdelbegin \DIFdel{Careful classifications of morphological features are thus essential if }\DIFdelend \DIFaddbegin \DIFadd{On the other hand, galactic bars and rings reveal details of slower, secular evolution and stellar orbital resonances; for example, bars are known to drive gas inwards and are related to the growth of a central bulge (reviews are given in \mbox{%DIFAUXCMD
\citealt{kor04}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citealt{mas11c}
}%DIFAUXCMD
). Information on morphological features is thus an essential element to understanding }\DIFaddend the assembly and evolution history of galaxies\DIFdelbegin \DIFdel{is to be fully understood}\DIFdelend .

\DIFdelbegin \DIFdel{Whereas traditional }\DIFdelend \DIFaddbegin \DIFadd{Traditional }\DIFaddend morphological classification relied on \DIFdelbegin \DIFdel{visual }\DIFdelend \DIFaddbegin \DIFadd{the careful }\DIFaddend inspection of small numbers of images by experts \citep[e.g., ][]{san61,dev91}\DIFaddbegin \DIFadd{. However}\DIFaddend , the sheer size of modern data sets\DIFaddbegin \DIFadd{, such as the SDSS MGS, }\DIFaddend make this approach impractical. Detailed classifications of \DIFdelbegin \DIFdel{SDSS images by experts }\DIFdelend \DIFaddbegin \DIFadd{limited subsets of SDSS images }\DIFaddend have been done \DIFdelbegin \DIFdel{by both }\DIFdelend \DIFaddbegin \DIFadd{though huge efforts of a small number of experts. }\DIFaddend \citet{fuk07} and \citet{bai11} \DIFdelbegin \DIFdel{, who }\DIFdelend determined modified Hubble types for samples of 2253 and 4458 galaxies, respectively\DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{, and the }\DIFaddend largest detailed professional effort to date was undertaken by \citet{nai10}, who provide classifications of 14,034 galaxies. Galaxy~Zoo~2 includes more than an order of magnitude more systems \DIFdelbegin \DIFdel{, each with }\DIFdelend \DIFaddbegin \DIFadd{than any of these efforts.  Furthermore, each galaxy has }\DIFaddend a large number of independent inspections\DIFaddbegin \DIFadd{, which permits estimates of the classification likelihood (and in some cases the strength of the feature in question)}\DIFaddend . The size of \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the GZ2 }\DIFaddend sample allows for a more complete study of small-scale morphological features and \DIFdelbegin \DIFdel{better statistics for rarer classes of objects, while having many independent classifications for each galaxy yields estimates of the classification likelihood (and possibly the relative feature strength)}\DIFdelend \DIFaddbegin \DIFadd{their correlation with many other galaxy properties (e.g. mass, stellar and gaseous content, environment), while providing better statistics for the rarest objects}\DIFaddend . 

The use of proxies \DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \DIFadd{for morphology -- }\DIFaddend such as colour, concentration index, spectral features, surface brightness profile, structural features, spectral energy distribution or some combination of these \DIFdelbegin \DIFdel{--- is }\DIFdelend \DIFaddbegin \DIFadd{-- has become common practice in astronomy. However these proxies are }\DIFaddend not an adequate substitute for \DIFdelbegin \DIFdel{complete }\DIFdelend \DIFaddbegin \DIFadd{full }\DIFaddend morphological classification. Each proxy has an unknown and possibly biased relation with the morphological features under study \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(e.g. most ellipticals are red and most spirals are blue, but interesting subsets of these morphological types are found with the opposite colour, \mbox{%DIFAUXCMD
\citealt{bam09,sch09,mas10a}
}%DIFAUXCMD
). }\DIFaddend With a sufficiently large set of galaxies, \DIFdelbegin \DIFdel{however, }\DIFdelend the diversity of the local population can be fully sampled, \DIFdelbegin \DIFdel{quantifying }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the relationship between morphology and the proxies discussed above \DIFaddbegin \DIFadd{can be fully identified}\DIFaddend . 

\DIFdelbegin \DIFdel{Despite recent advances in automated morphological classification }\DIFdelend \DIFaddbegin \DIFadd{Automated morphological classification is becoming much more sophisticated}\DIFaddend , driven in part by the availability of large training sets from the original Galaxy~Zoo \citep{ban10,hue11,dav13}\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. However }\DIFaddend these methods also do not \DIFdelbegin \DIFdel{currently replace }\DIFdelend \DIFaddbegin \DIFadd{yet provide an adequate substitute for }\DIFaddend classification by eye. In particular, as \citet{lin11} note, such efforts typically use proxies for morphology as their input \DIFdelbegin \DIFdel{, and so they suffer equally }\DIFdelend \DIFaddbegin \DIFadd{(especially colour), meaning they suffer }\DIFaddend from the objections raised above to the use of \DIFaddbegin \DIFadd{these }\DIFaddend morphological proxies. The release of the \DIFdelbegin \DIFdel{dataset associated with this paper }\DIFdelend \DIFaddbegin \DIFadd{Galaxy Zoo 2 dataset }\DIFaddend will be of interest to those developing such machine learning and computer vision systems. 

The GZ2 results were made possible by the participation of hundreds of thousands of volunteer `citizen scientists'. The original Galaxy~Zoo demonstrated the utility of this method in producing both large-scale catalogues as well as serendipitous discoveries of individual objects (see \citealt{lin11}\DIFaddbegin \DIFadd{, or Fortson et al. 2012 }\DIFaddend for a review of Galaxy~Zoo~1 results). Since then, this method has been expanded beyond simple \DIFaddbegin \DIFadd{galaxy }\DIFaddend shape classifications to include supernova identification \citep{smi11}, exoplanet discovery \citep{fis12,sch12} and a census of bubbles associated with star formation in the Milky~Way \citep{sim12a}\DIFaddbegin \DIFadd{, as well as a variety of ``big data" problems outside of astronomy}\footnote{\DIFadd{See }\url{http://www.thezooniverse.org} \DIFadd{for the full collection}}\DIFaddend . 

Several results based on early Galaxy Zoo 2 data have already been published. \citet{mas11c,mas12a} use GZ2 bar classifications to measure a clear increase in bar fraction for galaxies with redder colours, lower gas fractions, and more prominent bulges. \citet{hoy11} showed that the bars themselves are both redder and longer in redder disk galaxies. \citet{ski12} demonstrated that a significant correlation exists between barred and bulge-dominated galaxies at separations from \DIFdelbegin \DIFdel{0.15--3}\DIFdelend \DIFaddbegin \DIFadd{$0.15$--$3$}\DIFaddend ~Mpc. \citet{kav12a} used GZ2 to study early-type galaxies with visible dust lanes, while \citet{sim13} discovered a population of AGN host galaxies with no bulge, illustrating how black holes can grow and accrete via secular processes. Finally, \citet{cas13} quantify morphological signatures of interaction (including mergers, spiral arms, and bars) for galaxy pairs in the SDSS. 
...
An error in the selection query meant that the ``original'' sample initially missed objects \DIFdelbegin \DIFdel{flagged by the SDSS pipeline as having }\DIFdelend \DIFaddbegin \DIFadd{to which the SDSS photometric pipeline  \mbox{%DIFAUXCMD
\citep{2002AJ....123..485S}
}%DIFAUXCMD
assigned }\DIFaddend both \textsc{blended} and \textsc{child} \DIFdelbegin \DIFdel{morphologies. These }\DIFdelend \DIFaddbegin \DIFadd{flags. These are objects that have been deblended from a larger blend (hence }\textsc{\DIFadd{child}}\DIFadd{), and have been identified as blended themselves (hence }\textsc{\DIFadd{blended}}\DIFadd{; due to containing multiple peaks). However, these are `final' objects, as the SDSS deblender doesn't attempt to further deblend already deblended objects.  These }\DIFaddend galaxies, which are typically slightly brighter, larger and bluer than the general population, were added to the GZ2 site on 2009-09-02. These additional 28,174 galaxies are referred to as the ``extra'' sample. 
...
\DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{$N_{gal}$} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c}{$N_\mathrm{gal}$} \DIFaddendFL &
\DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{$N_{class}$} %%%
...
%DIFDELCMD < %%%
\DIFdelend \begin{equation}
\kappa = \DIFdelbegin \DIFdel{\frac{1}{N_r}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{N_\mathrm{r}}}\DIFaddend \sum\DIFdelbegin %DIFDELCMD < \limits%%%
...
\multicolumn{1}{l}{} &
\multicolumn{1}{l}{} &
\DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{$N_{task}\geq10$} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c}{$N_\mathrm{task}\geq10$} \DIFaddendFL &
\DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{$N_{task}\geq20$}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c}{$N_\mathrm{task}\geq20$}
\DIFaddendFL \\ 
\hline					
01                      & --        & --        & --        \\
02                      & 01        & 0.227     & 0.430     \\
...
\hline
 \end{tabular}
 \caption{Thresholds for determining well-sampled galaxies in GZ2. Thresholds depend on the number of votes for a classification task considered to be sufficient -- this table contains thresholds applied to previous task(s) for both 10 and 20 votes. As an example, to select galaxies that may or may not contain bars, cuts for \DIFdelbeginFL \DIFdelFL{$p_{features/disk}>0.430$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{features/disk}>0.430$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$p_{not~edgeon}>0.715$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{not~edgeon}>0.715$}\DIFaddendFL , and \DIFdelbeginFL \DIFdelFL{$N_{not~edgeon}\geq20$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$N_\mathrm{not~edgeon}\geq20$ }\DIFaddendFL should be applied. No thresholds are given for Tasks 01 and 06, since these are answered for every classification in GZ2. \label{tbl-thresholds}}
\end{table}
...
%DIF > K_{j,i} &=& \frac{\left(\frac{f_i}{f_j}\right)_{z=z^\prime} }{ \left(\frac{f_i}{f_j}\right)_{z=0}}
...
\caption{Comparison of GZ2 classifications for the coadded images of Stripe~82. Left: Distribution of the difference in vote fractions (\dcoadd) for galaxies that appear in both the coadd1 and coadd2 samples. Four example tasks are shown, including only galaxies with at least 10 responses per task. The dashed line shows the median of each distribution. The response ``noticeable bulge'' for Task~05 was the only example for which the mean \DIFdelbeginFL \DIFdelFL{$|\Delta_{coadd}| > 0.1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$|\Delta_\mathrm{coadd}| > 0.1$}\DIFaddendFL . Right: mean values \dcoadd~for every response in the GZ2 tree. 
\label{fig-stripe82_compare}}
\end{figure*}

Classifications for the two sets of coadded Stripe~82 images show no systematic differences for the majority of the GZ2 tasks. Figure~\ref{fig-stripe82_compare} shows the difference between the two vote fractions (\DIFdelbegin \DIFdel{$\Delta_{coadd} = p_{coadd1} - p_{coadd2}$}\DIFdelend \DIFaddbegin \DIFadd{$\Delta_\mathrm{coadd} = p_\mathrm{coadd1} - p_\mathrm{coadd2}$}\DIFaddend ) for four examples. A non-zero mean value of \dcoadd~would indicate a systematic bias in classification, possibly due to the differences in image processing. In GZ2, 33/37~tasks have \DIFdelbegin \DIFdel{$|\Delta_{coadd}| < 0.05$ }\DIFdelend \DIFaddbegin \DIFadd{$|\Delta_\mathrm{coadd}| < 0.05$ }\DIFaddend for galaxies with at least 10 responses to the task. 
...
Alternatively, utilisers of the catalogue can set their own thresholds for the debiased likelihoods to change the strength of the selection criteria. This flag is currently set via the combination of \DIFdelbegin \DIFdel{$p_{features/disk}>0.430$, $p_{edge-on,no}>0.715$, $p_{spiral,yes}>0.619$, $N_{spiral,yes}>20$, and $p_{3~arms}>0.8$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{features/disk}>0.430$, $p_\mathrm{edge-on,no}>0.715$, $p_\mathrm{spiral,yes}>0.619$, $N_\mathrm{spiral,yes}>20$, and $p_\mathrm{3~arms}>0.8$ }\DIFaddend (Table~\ref{tbl-thresholds}). These cuts are generally regarded as conservative, and more genuine three-armed spirals might be discovered by, for example, lowering the threshold on \DIFdelbegin \DIFdel{$p_{3~arms}$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{3~arms}$}\DIFaddend . If the number of objects returned by such a query is of a manageable size, we suggest that images be individually examined -- this is the only way to differentiate spirals with true radial symmetry from 2-armed spirals with an additional tidal tail, for example. Galaxy Zoo~2 papers that employ similar methods of selecting specific morphologies include \citet{mas11c}, \citet{kav12a}, \DIFdelbegin \DIFdel{and \mbox{%DIFAUXCMD
\citet{sim13}
}%DIFAUXCMD
...
The second common use case for the morphologies is the direct use of the likelihoods. While thresholds on the likelihoods are appropriate for some studies, many classifications do not exceed $p>0.8$ for any available response, especially when more than two responses are available. These intermediate classifications are a combination of genuine physical attributes of the galaxy (vote fractions of \DIFdelbegin \DIFdel{$p_{smooth}=0.5,p_{features/disk}=0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{smooth}=0.5,p_\mathrm{features/disk}=0.5$ }\DIFaddend may accurately characterize a galaxy with both strong bulge and disk components) in addition to limitations in accuracy from the image quality and variance among individual classifiers. The problem is that thresholding only samples a small portion of the vote distributions.

In order to use data for the {\it entire} sample, the debiased likelihoods for each response can be treated as probabilistic weights. As an example, consider the type fractions from Task~01 shown in Figure~\ref{fig-task01}. The left hand side shows the average fraction of morphological classes at each redshift only defining a ``class'' as exceeding some vote fraction threshold; as a result, more than half the galaxies are left ``unclassified'', with no strong majority. The panel on the right in Figure~\ref{fig-task01} uses the likelihoods directly. A galaxy with \DIFdelbegin \DIFdel{$p_{smooth}=0.6$, $p_{features/disk}=0.3$, and $p_{star/artifact}=0.1$ contributes 0.6 }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{smooth}=0.6$, $p_\mathrm{features/disk}=0.3$, and $p_\mathrm{star/artifact}=0.1$ contributes $0.6$ }\DIFaddend of a ``vote'' to smooth, \DIFdelbegin \DIFdel{0.3 }\DIFdelend \DIFaddbegin \DIFadd{$0.3$ }\DIFaddend to features/disk, and \DIFdelbegin \DIFdel{0.1 }\DIFdelend \DIFaddbegin \DIFadd{$0.1$ }\DIFaddend to star/artifact. This approach is generally suitable for studying morphology dependence on global variables, such as environment or \DIFdelbegin \DIFdel{colour}\DIFdelend \DIFaddbegin \DIFadd{color}\DIFaddend . Further examples of using the likelihoods as weights can be found in \citet{bam09}\DIFdelbegin \DIFdel{and \mbox{%DIFAUXCMD
\citet{ski12}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, \mbox{%DIFAUXCMD
\citet{ski12}
...
To assess both the scope and potential accuracy of the GZ2 classifications, we have compared our results to four morphological galaxy catalogues (including the previous version of Galaxy~Zoo). All four \DIFdelbegin \DIFdel{catalogues }\DIFdelend \DIFaddbegin \DIFadd{catalogs }\DIFaddend contain classifications based on optical SDSS images and have significant overlaps with the galaxies in GZ2. 
...
\hline
 \end{tabular}
 \caption{Comparison of the agreement in morphology between the GZ2 main sample and other \DIFdelbeginFL \DIFdelFL{catalogues}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{catalogs}\DIFaddendFL . For each category, a galaxy is considered to ``agree'' if it has a likelihood of at least 0.8 (``clean'') in GZ2 and at least 0.5 (``majority'') in the other \DIFdelbeginFL \DIFdelFL{catalogue }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{catalog }\DIFaddendFL (or for NA10, inclusion in the relevant category). This table gives both the total number of \DIFdelbeginFL \DIFdelFL{overlapping }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GZ2 }\DIFaddendFL galaxies and the fraction that agree in \DIFdelbeginFL \DIFdelFL{both catalogues}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the corresponding catalog}\DIFaddendFL . \label{tbl-compare}}
\end{table}
...
A summary of the agreement between GZ2 and other \DIFdelbegin \DIFdel{catalogues }\DIFdelend \DIFaddbegin \DIFadd{catalogs }\DIFaddend is given in Table~\ref{tbl-compare}; the remainder of this section discusses the results in more detail. 

\subsection{Galaxy~Zoo~1 and Galaxy~Zoo~2}\label{ssec-gz1gz2}

The galaxies in GZ2 are a subset of GZ1, with 248,883~in both \DIFdelbegin \DIFdel{catalogues}\DIFdelend \DIFaddbegin \DIFadd{catalogs}\DIFaddend . The similarities between GZ1 and Task~01~in GZ2 allow their results to be compared in detail. We analyzed vote fractions for the GZ1 ``elliptical'' category as compared to GZ2 ``smooth'' galaxies, and combined responses for all three GZ1 spiral categories to the GZ2 ``features or disk'' response. 

The matched GZ1-GZ2 catalogue contains 34,480 galaxies identified as ellipticals based on their debiased GZ1 likelihoods \citep{lin11}. Using the GZ2 debiased likelihoods, 50.4\% of galaxies have vote fractions exceeding 0.8 in both samples, while 97.6\% have vote fractions exceeding 0.5. There are only $\sim1200$ ellipticals identified in GZ1 that have \DIFdelbegin \DIFdel{$p_{features/disk}>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{features/disk}>0.5$ }\DIFaddend in GZ2. Of these, roughly 40\% are barred galaxies, and almost all show obvious bulges, the likely cause of their identification as early-type in GZ1. 

GZ2 contains 83,956 galaxies identified as spirals by GZ1. The agreement with the ``features or disk'' response in GZ2 is significantly lower than that of ellipticals. Only 31.6\% of the GZ1 clean spirals had vote fractions greater than in GZ2, with 59.2\% having a vote fraction greater than 0.5. The GZ2 debiased likelihoods for the same galaxies agree at 38.1\% (for 0.8) and 78.2\% (for 0.5). Of the $\sim1600$ spirals in GZ1 with \DIFdelbegin \DIFdel{$p_{features/disk}<0.4$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{features/disk}<0.4$}\DIFaddend , visual inspection shows these to be almost entirely inclined disks or lenticular galaxies without spiral arms. These galaxies are slightly bluer than the other early-types in GZ1; however, we emphasize that most elliptical galaxies with this colour \citep{sch09} were correctly classified in the initial GZ1 project. 

\begin{figure}
\includegraphics[angle=0,width=3.3in]{figures/gz1_gz2_trumpet.pdf}
\caption{Comparison of spiral galaxies using classifications for ``combined spiral'' (GZ1) and ``features or disk'' (GZ2). \DIFdelbeginFL %DIFDELCMD < {\it %%%
\DIFdelFL{Top}%DIFDELCMD < }%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Left}\DIFaddendFL : raw vote fractions. At intermediate values (\DIFdelbeginFL \DIFdelFL{$f_{sp}\sim0.5$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\mathrm{sp}\sim0.5$}\DIFaddendFL ), GZ1 classifiers are more likely to identify galaxies as spiral compared to GZ2. \DIFdelbeginFL %DIFDELCMD < {\it %%%
\DIFdelFL{Bottom}%DIFDELCMD < }%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Right}\DIFaddendFL : debiased vote fractions. At intermediate values, GZ1 and GZ2 classifications are consistent with each other; however, there is an increased scatter in the vote fractions near \DIFdelbeginFL \DIFdelFL{$p_{sp}\simeq0$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{sp}\simeq0$ }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{$p_{sp}\simeq1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{sp}\simeq1$}\DIFaddendFL . 
\label{fig-trumpet}}
\end{figure}

Figure~\ref{fig-trumpet} shows the difference between the vote fractions for the spiral classifications in GZ1 and features/disk classifications in GZ2 for all galaxies that appear in both catalogues. The vote fractions show a tight correlation at both very low and very high values of the GZ1 vote fraction for combined spiral (\DIFdelbegin \DIFdel{$f_{sp}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\mathrm{sp}$}\DIFaddend ), indicating that both projects agree on the strongest spirals (and corresponding ellipticals). At intermediate (0.2--0.8) values of \DIFdelbegin \DIFdel{$f_{sp}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\mathrm{sp}$}\DIFaddend , however, the GZ1 vote fractions are consistently higher than those in GZ2, differing by up to 0.25. When using debiased likelihoods in place of the vote fractions, this effect decreases dramatically; however, the tightness of the correlation correspondingly drops at low and high \DIFdelbegin \DIFdel{$p_{sp}$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{sp}$}\DIFaddend . 

\DIFdelbegin \DIFdel{Galaxies are slightly more likely to be identified as a spiral in GZ2 than in GZ1. }\DIFdelend Figure~\ref{fig-gz1_gz2} shows the distribution of the difference between \DIFdelbegin \DIFdel{spiral classifications}\DIFdelend \DIFaddbegin \DIFadd{the vote fractions for GZ1 and GZ2}\DIFaddend , using the debiased likelihoods for combined spirals for GZ1 and ``features or disk'' galaxies in GZ2. The slight \DIFdelbegin \DIFdel{leftward }\DIFdelend skew indicates that a galaxy is more likely to be identified as a spiral in GZ2 compared to GZ1. 
\DIFaddbegin 

\DIFadd{Galaxies are significantly more likely to be identified as a spiral in GZ1 than in GZ2. }\DIFaddend When restricted only to galaxies in the joint clean samples ($p>0.8$), the spread is greatly reduced and the distribution is centered around a difference of zero\DIFdelbegin \DIFdel{, indicating that the two agree very well for classifications with high levels of confidence. }\DIFdelend \DIFaddbegin \DIFadd{. The debiased vote fractions show a similar spread when comparing GZ1 and GZ2 classifications, although the skew toward spirals in GZ1 is largely removed. When using only clean galaxies and the debiased vote fractions, galaxies are more likely to be identified as spirals in GZ2. 
}\DIFaddend 
...
\caption{Comparison of the spiral feature vote fractions for objects in Galaxy~Zoo~1 (GZ1) and Galaxy~Zoo~2 (GZ2). The dashed line shows the difference between \DIFdelbeginFL \DIFdelFL{$p_{combined~spiral}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{combined~spiral}$ }\DIFaddendFL for GZ1 and \DIFdelbeginFL \DIFdelFL{$p_{features~or~disk}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{features~or~disk}$ }\DIFaddendFL for GZ2 for the 240,140 galaxies in both samples. The filled histogram shows the same metric for the 57,994 galaxies classified as ``clean'' spirals in both GZ1 and GZ2. 
\label{fig-gz1_gz2}}
\end{figure}

The GZ1 interface had one option to classify merging galaxies. This was a rare response, comprising less than 1~percent of the total type fraction at all redshifts in GZ1 \citep{bam09}. \citet{dar10a} found that a vote fraction of \DIFdelbegin \DIFdel{$f_{mg} > 0.6$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\mathrm{mg} > 0.6$ }\DIFaddend robustly identified merging systems in GZ1. Of the 1632 such systems classified in GZ2, more than 99\% were identified as ``odd'' galaxies, and 77.7\% had \DIFdelbegin \DIFdel{$p_{mg}>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{mg}>0.5$ }\DIFaddend in GZ2. This is partly due to early-stage merging spirals avoiding the ``merger'' classification, with only late-state mergers with extremely disturbed morphologies recording high vote fractions for the merger question. 
...
Comparison of \DIFaddbegin \DIFadd{the }\DIFaddend GZ2 \DIFdelbegin \DIFdel{to expert catalogues focuses }\DIFdelend \DIFaddbegin \DIFadd{data to the expert catalogues focusing }\DIFaddend on categories included in both classification schemes: bars, rings, and interacting galaxies. We also examine the relationship between the expert-determined T-types and the decision tree in GZ2.

\subsubsection{Bars}

To analyse the overlap between bars detected in expert classifications and \DIFaddbegin \DIFadd{and }\DIFaddend GZ2, we restrict comparisons to galaxies identified as possessing disks and being ``not edge-on'' (NEDs). NED galaxies are selected from the GZ2 data as having \DIFdelbegin \DIFdel{$p_{features/disk}>0.430, p_{not~edgeon}>=0.715$, and $N_{not~edgeon}\geq20$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{features/disk}>0.430, p_\mathrm{not~edgeon}>=0.715$, and $N_\mathrm{not~edgeon}\geq20$ }\DIFaddend (Table~\ref{tbl-thresholds}). This restricts the sample of overlapping galaxies in NA10 to 5,526 NEDs. The ``not edge-on'' cut is similar to a restriction on inclination angle of $\lesssim70^\circ$, based on the average axial ratio from the SDSS exponential profile fits.
...
Using the criteria for NED galaxies from Table~\ref{tbl-thresholds}, there are 1,543 galaxies with an EFIGI bar classification. Barred galaxies as identified by GZ2 (\DIFdelbegin \DIFdel{$p_{bar}\geq0.3$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{bar}\geq0.3$}\DIFaddend ) agree very well with EFIGI; less than 5\% of GZ2 barred galaxies have EFIGI attributes of 0, with a mean value of 0.56. This could indicate a selection preference toward medium-length bars (one-third to one-half of $D_{25}$), or could genuinely reflect the fact that medium bars are the most common length in disk galaxies. 

Data from both NA10 and EFIGI can be used to quantify a threshold to identify barred galaxies in GZ2 data. The fraction of non-barred galaxies in NEDs as identified by both expert \DIFdelbegin \DIFdel{catalogues }\DIFdelend \DIFaddbegin \DIFadd{catalogs }\DIFaddend drops to less than $5\%$ at a GZ2 vote fraction \DIFdelbegin \DIFdel{$p_{bar}=0.3$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{bar}=0.3$}\DIFaddend . This threshold may be changed depending on the specific science needs, but offers a useful trade-off between inclusion of nearly all (97\% from NA10) strong and intermediate bars and most (75\%) of the weak bars. This is a slightly more inclusive threshold than the $f\geq0.5$ used by \citet{mas11c}. \DIFaddbegin \DIFadd{We also note that the strong correlation between $p_\mathrm{bar}$ and EFIGI bar strength suggests that $p_\mathrm{bar}$ may be used directly (with caution) as a measure of bar strength in GZ2 galaxies.
}\DIFaddend 
...
In the NA10 catalogue, 18.2\% of all galaxies (30\% of disks) have a ring. Of those, 10\% are nuclear rings, 74\% are inner rings, and 32\% are outer rings (the sum is more than 100\% since one-third of ringed galaxies have multiple rings flagged). NA10 and GZ2 ring classifications are compared for the NEDs in both samples. No cut is applied to the vote fraction for the ``anything odd'' question; even a comparatively low cut of \DIFdelbegin \DIFdel{$p_{odd}>0.2$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{odd}>0.2$ }\DIFaddend eliminates roughly 40\% of the ringed galaxies identified in NA10. %We therefore analysed the GZ2 ring vote fraction for the entire sample of NEDs, with the caveat that some 13\% of these classifications are the result of three or fewer votes.

\begin{figure}
\includegraphics[angle=0,width=3.3in]{figures/na_gz2_rings.pdf}
\caption{Ring classifications in GZ2 and NA10. Data are for the 7,245 NED galaxies in both samples. Top: GZ2 vote fraction for rings (\DIFdelbeginFL \DIFdelFL{$N_{ring}/N_{odd}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$N_\mathrm{ring}/N_\mathrm{odd}$}\DIFaddendFL ) for all galaxies, split by their NA10 ring identifications. Bottom: GZ2 ring vote fraction for all rings identified by NA10, separated by ring type.
\label{fig-na_rings}}
\end{figure}

Figure~\ref{fig-na_rings} shows the distribution of the GZ2 ring vote fraction (\DIFdelbegin \DIFdel{$p_{ring}$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}$}\DIFaddend ) in the NED sample, split by the identification of a ring in NA10. While there is a marked increase in the fraction of ringed galaxies at \DIFdelbegin \DIFdel{$p_{ring}>0.5$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}>0.5$}\DIFaddend , more than a third of these galaxies are identified by NA10 as ringless. The agreement is significantly better if a limit is placed on the number of votes. Setting \DIFdelbegin \DIFdel{$N_{ring} > 5$}\DIFdelend \DIFaddbegin \DIFadd{$N_\mathrm{ring} > 5$}\DIFaddend , for example, increases the agreement to $\sim75\%$. 

The distribution of \DIFdelbegin \DIFdel{$p_{ring}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}$ }\DIFaddend is strongly affected by the ring type. Among galaxies that NA10 identifies as rings for which GZ2 strongly disagrees (\DIFdelbegin \DIFdel{$p_{ring}<0.5$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}<0.5$}\DIFaddend ), the majority are classified as inner rings. There are 308 ringed galaxies from NA10 that have no ring votes at all in GZ2; 84\% of these are inner rings. For galaxies on which the NA10 and GZ2 ring classifications agree, the percentage of outer ring galaxies is much higher. In the absence of specific instructions on different types of ring (the icon in Figure~\ref{fig-flowchart} does not indicate the size of the disk relative to the ring), GZ2 classifiers are much more likely to identify outer rings. The flat distribution of \DIFdelbegin \DIFdel{$p_{ring}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}$ }\DIFaddend for nuclear rings indicates that there is also no strong correlation between GZ2 classifications and ring structures in the bulge. 

Most galaxies with \DIFdelbegin \DIFdel{$p_{ring}>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}>0.5$ }\DIFaddend are classified as outer rings in NA10, especially if constraints on \DIFdelbegin \DIFdel{$N_{odd}$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\mathrm{odd}$ }\DIFaddend and/or \DIFdelbegin \DIFdel{$p_{odd}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{odd}$ }\DIFaddend are added. Part of the reason for the remaining disagreements may relate to the placement of the ring classification in GZ2 at the end of the tree, and only as a result of the user identifying something ``odd''. Without having seen examples of ring galaxies (especially as their structures connect to spiral arms), users may have been less likely to characterize the galaxy as odd and thus will not address the ring question. 

In EFIGI, rings are classified as inner, outer, and pseudo types. Both outer and pseudo ring galaxies show reasonably strong correlations with GZ2 ring classifications, with a mean ring vote fraction of 0.69 for outer ring galaxies and 0.71 for pseudo-ring galaxies. The mean GZ2 ring vote fraction for inner rings is only 0.41. For galaxies in both EFIGI and GZ2, a high GZ2 ring vote fraction agrees significantly with the expert classification of a ring. 89\% of galaxies with \DIFdelbegin \DIFdel{$p_{ring}>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{ring}>0.5$ }\DIFaddend and having at least 10 votes for ``Anything odd?'' were classified as rings in EFIGI. 
...
\caption{Merger classifications in GZ2 and NA10. Data are for the 3,878 galaxies in both samples with \DIFdelbeginFL \DIFdelFL{$p_{odd}>0.223$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{odd}>0.223$}\DIFaddendFL , showing the distribution of the vote fraction for the ``merger'' response to Task~08 in GZ2. The majority of galaxies have \DIFdelbeginFL \DIFdelFL{$p_{merger}<0.1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{merger}<0.1$}\DIFaddendFL . Galaxies classified by NA10 both as disturbed and in pairs dominate at \DIFdelbeginFL \DIFdelFL{$p_{merger}>0.5$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\mathrm{merger}>0.5$}\DIFaddendFL , but there remains a significant population of undisturbed galaxies even at the highest GZ2 vote fractions.
\label{fig-na_pairs}}
\end{figure}

Figure~\ref{fig-na_pairs} shows the distributions of NA10 paired and interacting galaxies with at least 10 votes for ``yes'' (something odd) for Task~06. Most galaxies have no votes for a merger, with only 6\% of galaxies having \DIFdelbegin \DIFdel{$N_{merger}\ge5$}\DIFdelend \DIFaddbegin \DIFadd{$N_\mathrm{merger}\ge5$}\DIFaddend . The numbers of both paired and interacting galaxies identified by NA10 begin to exceed the non-interacting population at a merger fraction above \DIFdelbegin \DIFdel{$p_{merger}>0.25$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{merger}>0.25$}\DIFaddend . There is a significant population of non-interacting galaxies up to very high GZ2 vote fractions, however, which means that a simple cutoff is insufficient to produce a pure merger population by this criteria. 

We visually examined galaxies that have high GZ2 merger fractions (\DIFdelbegin \DIFdel{$p_{merger}>0.5$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{merger}>0.5$}\DIFaddend ) but are classified by NA10 as non-interacting. The majority of these galaxies show obvious nearby companions, many of which appear to be tidally stripped or otherwise deformed. Some of these galaxies are likely the result of projection effects and are not truly interacting pairs -- however, a significant fraction may be true interactions not identified in NA10. The contrary case (galaxies identified as interacting by NA10, but \DIFdelbegin \DIFdel{$p_{merger}<0.1$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{merger}<0.1$ }\DIFaddend in GZ2), generally show faint extended features -- mostly shells and tidal tails -- that are clear signs of interacting. Most of these galaxies have no apparent companion visible in the image, however.
...
EFIGI has no dedicated category for mergers; galaxies are classified on whether they have any close companions (``contamination'') or distortions in the galaxy profile (``perturbation''), which may or may not be merger-related. Galaxies cleanly classified by GZ2 as mergers are only weakly correlated with both attributes; the mean EFIGI value in GZ2 mergers is 0.31 for the perturbation attribute and 0.48 for contamination. Figure~\ref{fig-efigi_mergers} shows \DIFaddbegin \DIFadd{that while there is }\DIFaddend only a very weak correlation ($\rho=0.14$) between EFIGI perturbation and GZ2 merger vote fraction. Highly-perturbed galaxies with low GZ2 \DIFdelbegin \DIFdel{$p_{merger}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{merger}$ }\DIFaddend are mostly dwarf peculiar and irregular galaxies with no sign of tidal features or an interacting companion. 
...
The left side of Figure~\ref{fig-spiraltightness} shows the distribution of NA10 T-types for galaxies based on their GZ2 vote fractions for winding arms. Vote fractions for both tight and medium winding arms are relatively normally distributed, with the mean \DIFdelbegin \DIFdel{$p_{tight}=0.46$ and $p_{medium}=0.37$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{tight}=0.46$ and $p_\mathrm{medium}=0.37$}\DIFaddend . Strongly-classified loose spirals are much rarer, with 75\% of galaxies having \DIFdelbegin \DIFdel{$p_{loose}<0.2$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{loose}<0.2$}\DIFaddend . Almost no elliptical galaxies from the NA10 catalogue are included in the NED sample, although there are significant numbers of S0 galaxies. 

For tight spirals, the category of galaxies with the highest vote fractions has more earlier-type spirals than galaxies with a low vote for tight spiral winding arms. For a tight spiral vote fraction above 0.9, 85\% of galaxies are Sb or earlier. Medium-wound spirals with high vote fractions tend to be Sb and Sc -- the proportion of both types increases as a function of \DIFdelbegin \DIFdel{$p_{medium}$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{medium}$}\DIFaddend , and constitute 84\% of galaxies when \DIFdelbegin \DIFdel{$p_{medium}>0.6$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{medium}>0.6$}\DIFaddend . Galaxies classified as strongly medium-wound are rare, however, with only 23 galaxies having \DIFdelbegin \DIFdel{$p_{medium}>0.8$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{medium}>0.8$}\DIFaddend .  Loose spirals are dominated by Sc and Sd galaxies at high vote fractions, comprising more than 50\% of galaxies with \DIFdelbegin \DIFdel{$p_{loose}>0.7$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{loose}>0.7$}\DIFaddend . \citet{cas13} found that galaxies with high \DIFdelbegin \DIFdel{$p_{loose}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{loose}$ }\DIFaddend often show tidal features and host a significant proportion of interacting galaxies. This distribution may reflect the experimental design of GZ2, with volunteers preferring extreme ends of a distribution rather than an indistinct `central' option. 
...
EFIGI T-types also correlate strongly with GZ2 bulge dominance. More than 90\% of galaxies with \DIFdelbegin \DIFdel{$p_{no bulge}>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{no bulge}>0.5$ }\DIFaddend are late-type spirals, with the bulk of these Sd galaxies. Both \DIFdelbegin \DIFdel{$p_{just noticeable}$ and $p_{obvious}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{just noticeable}$ and $p_\mathrm{obvious}$ }\DIFaddend show a continuum of T-types as the vote fractions increase, with Sc and Sd galaxies having high vote fractions for the former and S0, Sa, and Sb galaxies in the latter. Galaxies with high vote fractions for \DIFdelbegin \DIFdel{$p_{dominant}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{dominant}$ }\DIFaddend are primarily S0s, along with a few elliptical galaxies that had enough votes as disk galaxies in GZ2 to answer the bulge classification question. 
...
Finally, we note that \citet{sim13} identified a significant effect in which nuclear point sources, such as AGN, can mimic bulges in the GZ2 classifications. This has not been accounted for in this analysis, but could potentially be addressed by separating \DIFaddbegin \DIFadd{the sample into }\DIFaddend AGN and quiescent galaxies \DIFdelbegin \DIFdel{via optical emission lines \mbox{%DIFAUXCMD
\citep{bal81}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(via BPT line ratios) }\DIFaddend and looking for systematic differences between the two samples. 
...
Figure~\ref{fig-efigi_bulgearms} (left panels) show the relationship between \bt~and the GZ2 bulge dominance vote fractions for NED galaxies. \DIFdelbegin \DIFdel{$p_{obvious}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{obvious}$ }\DIFaddend is strongly correlated ($\rho=0.65$) with \bt, while \DIFdelbegin \DIFdel{$p_{just~noticeable}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{just~noticeable}$ }\DIFaddend has a nearly equal and opposite anti-correlation. Very few galaxies in the sample have either \DIFdelbegin \DIFdel{$p_{no~bulge}>0$ or $p_{dominant}>0$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{no~bulge}>0$ or $p_\mathrm{dominant}>0$}\DIFaddend , but those that do show corresponding changes in the EFIGI \bt. In particular, the number of galaxies with $B/T=0$ and \DIFdelbegin \DIFdel{$p_{just~noticeable}>0$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{just~noticeable}>0$ }\DIFaddend reinforces the results of \citet{sim13}, who showed that GZ2 bulge prominences increase with the presence of central point sources in the image (such as AGN). 
...
The EFIGI arm curvature classifications broadly follow the trends expected from matching targets with GZ2. \DIFdelbegin \DIFdel{$p_{tight}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{tight}$ }\DIFaddend is the most strongly correlated with the EFIGI arm curvature parameter (Figure~\ref{fig-efigi_bulgearms}, right panels). The Spearman's correlation coefficient for tight spirals is $\rho=0.62$. The medium spiral vote fraction is clustered in the middle of the EFIGI values, where galaxies with the highest GZ2 vote fraction have EFIGI values of 0.25--0.50, with $\rho=-0.26$. Loose spirals shows an anti-correlation ($\rho=-0.54$); very few galaxies have GZ2 vote fractions above 0.5, but those which do have low EFIGI arm curvature values at 0.25 or below. 
...
The relationship between bulge dominance and late-type probability is much stronger for the two intermediate responses for GZ2. Galaxies for which \DIFdelbegin \DIFdel{$p_{just~noticeable}>0.6$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{just~noticeable}>0.6$ }\DIFaddend have a rapid increase in their late-type probabilities, with a sharp transition from the constant late-type probability between 0.25 and 0.6. As expected, the opposite effect occurs for obvious bulges; a vote fraction of \DIFdelbegin \DIFdel{$p_{obvious}<0.2$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{obvious}<0.2$ }\DIFaddend gives a very strong probability of being an Scd galaxy, while galaxies with \DIFdelbegin \DIFdel{$p_{obvious}>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{obvious}>0.5$ }\DIFaddend are favored to be classified as Sb or earlier. 

\begin{figure}
\includegraphics[angle=0,width=3.3in]{figures/hc_gz2_bulge_contour.pdf}
\caption{\citet{hue11} late-type spiral probability as a function of the GZ2 vote fraction for bulge dominance. The colour of the contours is log~\DIFdelbeginFL \DIFdelFL{$(N_{gal} + 1)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(N_\mathrm{gal} + 1)$}\DIFaddendFL , where \DIFdelbeginFL \DIFdelFL{$N_{gal}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$N_\mathrm{gal}$ }\DIFaddendFL ranges from 0 to $1.5\times10^3$. Data are the 54,987 NED galaxies appearing in both GZ2 and HC11.
\label{fig-hc_gz2_bulge_contour}}
\end{figure}

Finally, we examined the potential effect of bars on the automated classifications. Figure~\ref{fig-hc_gz2_bar} shows the average HC11 probability as a function of GZ2 \DIFdelbegin \DIFdel{$p_{bar}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{bar}$ }\DIFaddend for NED galaxies. The relative proportions of galaxies as classified by HC11 is flat as a function of GZ2 \DIFdelbegin \DIFdel{$p_{bar}$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathrm{bar}$}\DIFaddend , with 31\% for early-type and 69\% late-type. The presence of a bar thus does not strongly affect automated classifications, at least \DIFdelbegin \DIFdel{as }\DIFdelend far as distinguishing early- from late-type galaxies. 
...
The final \DIFdelbegin \DIFdel{catalogue }\DIFdelend \DIFaddbegin \DIFadd{catalog }\DIFaddend consists of five tables, comprising morphological classifications for the GZ2 main sample (separated into galaxies with spectroscopic and photometric redshifts) and galaxies from Stripe~82 (for normal-depth and two sets of coadded images with spectroscopic redshifts). Data for each galaxy includes \DIFdelbegin \DIFdel{(for each response) the }\DIFdelend \DIFaddbegin \DIFadd{both }\DIFaddend raw and weighted number of votes \DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{for each response, }\DIFaddend raw and weighted vote fractions \DIFaddbegin \DIFadd{for each response}\DIFaddend , the debiased vote fraction, and an optional flag which indicates if a feature \DIFdelbegin \DIFdel{has been }\DIFdelend \DIFaddbegin \DIFadd{meets sufficient thresholds to be }\DIFaddend robustly identified. Portions of the data are presented in Tables~\ref{tbl-mainclass}--\ref{tbl-stripe82_coadd2}; full machine-readable tables are available at \url{http://data.galaxyzoo.org} and in SDSS Data Release~10. 

We have compared the GZ2 classifications in detail to several other morphological \DIFdelbegin \DIFdel{catalogues}\DIFdelend \DIFaddbegin \DIFadd{catalogs}\DIFaddend . Early and late-type classifications are consistent with results from the original Galaxy~Zoo, especially for galaxies in the clean samples. Expert \DIFdelbegin \DIFdel{catalogues }\DIFdelend \DIFaddbegin \DIFadd{catalogs }\DIFaddend \citep{nai10,bai11} show good agreement for galaxies with medium to strong bars; GZ2 is less confident in identifying expert-classified weak and/or nuclear bars. In ringed galaxies, GZ2 recovers the majority of outer rings, but relatively few inner or nuclear rings due to the design of the GZ2 question. Pairs and interacting galaxies are more difficult to reliably cross-match in a clean sample, although \citet{cas13} have already shown that the GZ2 ``loose winding arms'' parameter is a reliable proxy for interaction. The GZ2 bulge dominance parameter strongly correlates with the Hubble T-type from both expert \DIFdelbegin \DIFdel{catalogues}\DIFdelend \DIFaddbegin \DIFadd{catalogs}\DIFaddend . Adding GZ2 measurements of the spiral arm tightness, though, does not increase the T-type classification accuracy. Automated classifications from \citet{hue11} agree well with GZ2 in separating elliptical and late-type spirals, although identification of S0 galaxies still represents a challenge. 

GZ2 contains more than an order of magnitude more galaxies than the largest comparable expert-classified catalogues (NA10, EFIGI) while still classifying detailed morphological features not replicable by automated classifications. GZ2 data have already been used to demonstrate a relationship between bar fraction and the colour, gas fractions, and bulge size of disk galaxies \citep{mas11c,mas12a}, as well as studies of the bar \DIFdelbegin \DIFdel{colour }\DIFdelend \DIFaddbegin \DIFadd{color }\DIFaddend and length itself \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{hoy11}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{hoy11}
}%DIFAUXCMD
}\DIFaddend . The size of the \DIFdelbegin \DIFdel{catalogues }\DIFdelend \DIFaddbegin \DIFadd{catalogs }\DIFaddend has allowed for the discovery and study of comparatively rare objects, such as early-type dust lane galaxies \citep{kav12a} and bulgeless AGN hosts \citep{sim13}. Direct use of the GZ2 likelihoods has also been used to quantify the environmental dependence on morphology, showing a correlation for barred and bulge-dominated galaxies \citep{ski12} and identifying reliable signatures of interaction from GZ2 data \citep{cas13}. 

The \DIFdelbegin \DIFdel{success }\DIFdelend \DIFaddbegin \DIFadd{scientific productivity }\DIFaddend of the Galaxy \DIFdelbegin \DIFdel{~}\DIFdelend Zoo project has already shown that \DIFdelbegin \DIFdel{volunteer classification }\DIFdelend \DIFaddbegin \DIFadd{the use of multiple independent volunteer classifications }\DIFaddend is a robust method for \DIFaddbegin \DIFadd{the }\DIFaddend analysis of large data sets \DIFdelbegin \DIFdel{; this }\DIFdelend \DIFaddbegin \DIFadd{of galaxy images. This }\DIFaddend public release of the detailed Galaxy \DIFdelbegin \DIFdel{~Zoo ~}\DIFdelend \DIFaddbegin \DIFadd{Zoo }\DIFaddend 2 catalogue intends to build on this \DIFdelbegin \DIFdel{further, both from their inherent scientific value and as }\DIFdelend \DIFaddbegin \DIFadd{success, by demonstrating the reliability and benefit of these classifications over both expert classifications and automated classifications. We publicy release these Galaxy Zoo 2 classifications both as a rich data set to exploit to study galaxy evolution, and as }\DIFaddend training sets for refining \DIFdelbegin \DIFdel{methods of automatic }\DIFdelend \DIFaddbegin \DIFadd{method of automated }\DIFaddend classification techniques.
...
\DIFdelbegin \DIFdel{Please contact the author(s) to request access to research materials discussed in this paper. 
}\DIFdelend 

The development of Galaxy Zoo 2 was supported by The Leverhulme Trust. \DIFdelbegin \DIFdel{KWW and LFF would like to acknowledge support from the US National Science Foundation 
under grant DRL-0941610. }\DIFdelend CJL acknowledges support from the \DIFdelbegin \DIFdel{Science and Technology Facilities Council (STFC ) }\DIFdelend \DIFaddbegin \DIFadd{STFC }\DIFaddend Science in Society program. KS gratefully acknowledges support from Swiss National Science Foundation Grant PP00P2\_138979/1. 
\DIFdelbegin \DIFdel{TM acknowledges funding from the STFC ST/J500665/1. RCN was partially supported by STFC grant ST/K00090X/1. BDS acknowledges supportfrom Worcester College, Oxford, and from the Oxford Martin School program on computational cosmology}\DIFdelend \DIFaddbegin 

\DIFadd{Boilerplate for individual grants, Zooniverse support, etc.
}

\DIFadd{STFC numbers for everyone in the UK}\DIFaddend .
...
%DIFDELCMD <  &  &  &  & 
%DIFDELCMD < \multicolumn{6}{c}{\underline{t01\_smooth\_or\_features\_a01\_smooth\_}} &
%DIFDELCMD < \multicolumn{6}{c}{\underline{t01\_smooth\_or\_features\_a02\_features\_or\_disk\_}} &
%DIFDELCMD < \colhead{$\dots$}
%DIFDELCMD < \\
%DIFDELCMD < \colhead{SDSS DR7 objID} & 
%DIFDELCMD < \colhead{sample} & 
%DIFDELCMD < \colhead{$N_{class}$} & 
%DIFDELCMD < \colhead{$N_{votes}$} & 
%DIFDELCMD < \colhead{count} & 
%DIFDELCMD < \colhead{wt\_count} & 
%DIFDELCMD < \colhead{fraction} & 
%DIFDELCMD < \colhead{wt\_fraction} & 
%DIFDELCMD < \colhead{debiased} &
%DIFDELCMD < \colhead{flag} &
%DIFDELCMD < \colhead{count} & 
%DIFDELCMD < \colhead{weight} & 
%DIFDELCMD < \colhead{fraction} & 
%DIFDELCMD < \colhead{wt\_fraction} & 
%DIFDELCMD < \colhead{debiased} &
%DIFDELCMD < \colhead{flag} &
...
%DIFDELCMD < \colhead{$\dots$}
%DIFDELCMD < \\
%DIFDELCMD < \colhead{Stripe82 objID} & 
%DIFDELCMD < \colhead{$N_{class}$} & 
%DIFDELCMD < \colhead{$N_{votes}$} & 
%DIFDELCMD < \colhead{count} & 
%DIFDELCMD < \colhead{wt\_count} & 
...
%DIF > \newpage
%DIF > \clearpage
%DIF > \tabletypesize{\scriptsize}
%DIF > \begin{deluxetable}{lccrrrr}
%DIF > \tablecolumns{16}
%DIF > \tablewidth{0pc}
%DIF > \tablecaption{GZ2 vote fractions for the main and Stripe~82 normal-depth samples \label{tbl-stripe82compare}}
%DIF > \tabletypesize{\scriptsize}
%DIF > \tablehead{
%DIF >  & \multicolumn{2}{c}{\underline{Mean vote fraction}} & \multicolumn{2}{c}{\underline{Number of galaxies}} & \multicolumn{2}{c}{\underline{Sample comparison}} \\
%DIF > \colhead{User question and response} & 
%DIF > \colhead{Main} & 
%DIF > \colhead{Stripe~82} & 
%DIF > \colhead{Main} & 
%DIF > \colhead{Stripe~82} &
%DIF > \colhead{$f_\mathrm{m} - f_\mathrm{s}$} & 
%DIF > \colhead{$f_\mathrm{m}/f_\mathrm{s}$} 
%DIF > }
%DIF > \small
%DIF > \startdata
%DIF > T01\_SMOOTH\_OR\_FEATURES\_A01\_SMOOTH                      &      0.642 &      0.613 &     273783 &       8437 &       0.029 &       1.048 \\
%DIF > T01\_SMOOTH\_OR\_FEATURES\_A02\_FEATURES\_OR\_DISK          &      0.325 &      0.352 &     273783 &       8437 &      -0.027 &       0.924 \\
%DIF > T01\_SMOOTH\_OR\_FEATURES\_A03\_STAR\_OR\_ARTIFACT          &      0.033 &      0.036 &     273783 &       8437 &      -0.003 &       0.924 \\
%DIF > T02\_EDGEON\_A04\_YES                                       &      0.234 &      0.229 &     118147 &       4110 &       0.005 &       1.020 \\
%DIF > T02\_EDGEON\_A05\_NO                                        &      0.766 &      0.771 &     118147 &       4110 &      -0.005 &       0.994 \\
%DIF > T03\_BAR\_A06\_BAR                                          &      0.287 &      0.292 &      87647 &       3077 &      -0.004 &       0.986 \\
%DIF > T03\_BAR\_A07\_NO\_BAR                                      &      0.713 &      0.708 &      87647 &       3077 &       0.004 &       1.006 \\
%DIF > T04\_SPIRAL\_A08\_SPIRAL                                    &      0.660 &      0.674 &      87621 &       3077 &      -0.014 &       0.980 \\
%DIF > T04\_SPIRAL\_A09\_NO\_SPIRAL                                &      0.340 &      0.326 &      87621 &       3077 &       0.014 &       1.042 \\
%DIF > T05\_BULGE\_PROMINENCE\_A10\_NO\_BULGE                      &      0.129 &      0.114 &      87608 &       3076 &       0.015 &       1.129 \\
%DIF > T05\_BULGE\_PROMINENCE\_A11\_JUST\_NOTICEABLE               &      0.479 &      0.491 &      87608 &       3076 &      -0.012 &       0.976 \\
%DIF > T05\_BULGE\_PROMINENCE\_A12\_OBVIOUS                        &      0.321 &      0.333 &      87608 &       3076 &      -0.011 &       0.966 \\
%DIF > T05\_BULGE\_PROMINENCE\_A13\_DOMINANT                       &      0.070 &      0.062 &      87608 &       3076 &       0.008 &       1.134 \\
%DIF > T06\_ODD\_A14\_YES                                          &      0.199 &      0.185 &     273636 &       8391 &       0.014 &       1.073 \\
%DIF > T06\_ODD\_A15\_NO                                           &      0.801 &      0.815 &     273636 &       8391 &      -0.014 &       0.983 \\
%DIF > T07\_ROUNDED\_A16\_COMPLETELY\_ROUND                        &      0.330 &      0.326 &     229974 &       6956 &       0.005 &       1.014 \\
%DIF > T07\_ROUNDED\_A17\_IN\_BETWEEN                              &      0.496 &      0.501 &     229974 &       6956 &      -0.004 &       0.991 \\
%DIF > T07\_ROUNDED\_A18\_CIGAR\_SHAPED                            &      0.173 &      0.174 &     229974 &       6956 &      -0.000 &       0.999 \\
%DIF > T08\_ODD\_FEATURE\_A19\_RING                                &      0.147 &      0.178 &      72439 &       2114 &      -0.031 &       0.827 \\
%DIF > T08\_ODD\_FEATURE\_A20\_LENS\_OR\_ARC                       &      0.045 &      0.040 &      72439 &       2114 &       0.005 &       1.122 \\
%DIF > T08\_ODD\_FEATURE\_A21\_DISTURBED                           &      0.121 &      0.125 &      72439 &       2114 &      -0.003 &       0.973 \\
%DIF > T08\_ODD\_FEATURE\_A22\_IRREGULAR                           &      0.177 &      0.170 &      72439 &       2114 &       0.007 &       1.039 \\
%DIF > T08\_ODD\_FEATURE\_A23\_OTHER                               &      0.282 &      0.245 &      72439 &       2114 &       0.037 &       1.152 \\
%DIF > T08\_ODD\_FEATURE\_A24\_MERGER                              &      0.212 &      0.209 &      72439 &       2114 &       0.003 &       1.015 \\
%DIF > T08\_ODD\_FEATURE\_A38\_DUST\_LANE                          &      0.016 &      0.034 &      72439 &       2114 &      -0.018 &       0.475 \\
%DIF > T09\_BULGE\_SHAPE\_A25\_ROUNDED                             &      0.538 &      0.567 &      21496 &        746 &      -0.029 &       0.948 \\
%DIF > T09\_BULGE\_SHAPE\_A26\_BOXY                                &      0.103 &      0.096 &      21496 &        746 &       0.007 &       1.077 \\
%DIF > T09\_BULGE\_SHAPE\_A27\_NO\_BULGE                           &      0.359 &      0.338 &      21496 &        746 &       0.022 &       1.065 \\
%DIF > T10\_ARMS\_WINDING\_A28\_TIGHT                              &      0.370 &      0.392 &      55800 &       2052 &      -0.022 &       0.943 \\
%DIF > T10\_ARMS\_WINDING\_A29\_MEDIUM                             &      0.417 &      0.405 &      55800 &       2052 &       0.011 &       1.028 \\
%DIF > T10\_ARMS\_WINDING\_A30\_LOOSE                              &      0.214 &      0.203 &      55800 &       2052 &       0.011 &       1.054 \\
%DIF > T11\_ARMS\_NUMBER\_A31\_1                                   &      0.074 &      0.067 &      55805 &       2053 &       0.008 &       1.117 \\
%DIF > T11\_ARMS\_NUMBER\_A32\_2                                   &      0.501 &      0.478 &      55805 &       2053 &       0.023 &       1.048 \\
%DIF > T11\_ARMS\_NUMBER\_A33\_3                                   &      0.093 &      0.098 &      55805 &       2053 &      -0.005 &       0.950 \\
%DIF > T11\_ARMS\_NUMBER\_A34\_4                                   &      0.038 &      0.043 &      55805 &       2053 &      -0.005 &       0.884 \\
%DIF > T11\_ARMS\_NUMBER\_A36\_MORE\_THAN\_4                       &      0.030 &      0.033 &      55805 &       2053 &      -0.002 &       0.934 \\
%DIF > T11\_ARMS\_NUMBER\_A37\_CANT\_TELL                          &      0.263 &      0.281 &      55805 &       2053 &      -0.019 &       0.934 \\
%DIF > \enddata
%DIF > \tablecomments{Data for each task is only for galaxies with at least 10 total responses to the classification question.}
%DIF > \end{deluxetable}
\DIFaddend 
